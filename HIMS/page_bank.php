...
$str=mysql_query("SELECT * from bank_master WHERE bank_name like '%".$_POST['t1']."%'");
}
else
{
	$str=mysql_query("SELECT * from bank_master");
}
...
$output .='<table class="table" bordered="1">
    ...
    while($row = mysql_fetch_array($str)) {
		...
		<td> '.$row['bank_status'].'</td></tr>';		

		}
		$output .= '</table>';
			header("Content-Type: application/xls");
			header("Content-Disposition: attachment; filename=banks.xls");
			echo $output;
}
...
echo "<table border='1' class='act'>";
	...
	<td align='center'>Bank Status</td>	
	</tr>";
	$i=1;
    while($row = mysql_fetch_array($str)) {    	
    	?>
