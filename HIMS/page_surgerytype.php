...
$str=mysql_query("SELECT * from surgerytype_master WHERE st_name like '%".$_POST['t1']."%'");
}
else
{
	$str=mysql_query("SELECT * from surgerytype_master");
}
echo "<table border='1'>";
	echo "<tr>
	<td>S.no</td>
...
$output .='<table class="table" bordered="1">
    ...
    while($row = mysql_fetch_array($str)) {
		...
		<td> '.$row['st_code'].'</td>		
		<td> '.$row['st_name'].'</td>
		<td> '.$row['st_status'].'</td></tr>';		

		}
		$output .= '</table>';
			header("Content-Type: application/xls");
			header("Content-Disposition: attachment; filename=surgerytype.xls");
			echo $output;
}
...
echo "<table border='1'>";
	...
	<td align='center'> <?php  echo $row['st_code'] ?></td>
	<td align='center'><?php  echo $row['st_name'] ?></td>
	<td align='center'><?php  echo $row['st_status'] ?></td>
	</tr>
<?php
}
echo "</table>";
